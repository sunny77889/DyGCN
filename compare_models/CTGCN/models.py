...
from layers import MLP, CoreDiffusion
...
class CTGCN(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim,ip_num, trans_num, diffusion_num, duration, bias=True, rnn_type='GRU', model_type='C', trans_activate_type='L'):
    ...
    def forward(self, x_list, adj_list, mask, device):

        hx_list, trans_list = [], []
        node_feats=torch.zeros(self.ip_num, 5, self.output_dim).to(device)
            
        for i in range(len(adj_list)):
            x = self.mlp_list[i](x_list[i])
            trans_list.append(x)
            x = self.duffision_list[i](x, adj_list[i])
            node_feats[:,i,:][mask[i]]=x.squeeze()
        out, _ = self.rnn(node_feats)
