...
from data_process import (Cic2018_Dataset, Cic_Dataset, UNSW_Dataset,
                          USTC_Dataset)
from egcn_h import EGCN as EH
from egcn_o import EGCN as EO
...
def train(args, seq_len,graphs, node_feats, device, train_len, epochs):
    model = EO(args, activation = torch.nn.RReLU(), device=device)
    cls=Classifier(args)
    cls.to(device)
    ...
    for i in range(epochs):
        ...
        for idx in tqdm(range(train_len[0],train_len[1]-seq_len)):
            ...
            nx=[node_feats[g['node_idx']] for g in gs]
            mask_list=[g['node_idx'] for g in gs]
            out=model(adjs, nx, mask_list)
...
if __name__ == '__main__':
    ...
    # train(args, seq_len, graphs=graphs, node_feats=node_feats.to(device), device=device, train_len=[0, 529], epochs=1) #cic[0,529], unsw[200:600], ustc[10:100]
    # t2=time()
    # print('训练时间', t2-t1)
    # predict(args, graphs=graphs, node_feats=node_feats.to(device))
    # t3=time()
    # print('测试时间', t3-t2)   
